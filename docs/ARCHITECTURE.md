...
> **Кому что читать.** Разработчику компонента, которому надо подключить свои
> тесты, — [HOWTO.md](HOWTO.md): четыре шага и готовые примеры для .NET и C++,
> без устройства системы. Этот документ — про устройство и обоснования решений,
> он для тех, кто принимает по нему решения.

**Коротко.** Разработчик кладёт в свой репозиторий три файла — описание окружения,
...
Возьмём то, как это устроено у пилота сегодня — эта картина типична.
...
из которых ломается молча: сертификат протухает — перестаёт собираться у всех,
и никто не понимает почему.

Но главное не это. **На выходе нет ничего.** Команда запуска —
`dotnet test --filter … --no-build` — идёт без ключей выгрузки отчётов, а томов
под них в compose нет. Результаты остаются внутри контейнера и умирают вместе с
...
всё прошло, и когда упало, — и обязательно **до** уборки. Иначе разбирать
падение было бы нечем.
...
Отсюда — как расширять, не трогая общий код:
...
У пилота секретов нет вообще (там тестовые креды), поэтому первый прогон этот
...
**Правки их репозитория: ноль.** Всё изменение — три файла профиля: `build:`
заменён на готовый образ, убраны `container_name` и `restart: always`, к команде
запуска добавлены три ключа выгрузки отчётов.

Дорога до зелёного дала семь дефектов, каждый из которых выстрелил бы на первом
настоящем билде и был бы неотличим от «тесты сломались»: невозможность записи
отчётов, неспособность базы стартовать под снятыми привилегиями, отсутствие
плагина compose на агентах, удвоение цифр покрытия из-за того, что VSTest
пишет отчёт дважды, отсутствие причины в сообщении о падении, исчерпание диска
под видом падения тестов, и зависимость результата от содержимого рабочего
каталога.
...
- **Доступ к docker-сокету.** Второй набор тестов пилота создаёт контейнеры
  ...
  агент течёт. В первом этапе такие наборы не поддерживаются. Дальнейший путь —
...
./run_local.sh <набор> --repo <репо>     # настоящий набор конкретной команды
```
