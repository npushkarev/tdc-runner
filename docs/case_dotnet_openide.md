...
набор на PostgreSQL. Всё, что ниже, — не проект, а факты: прогнано на dev-стенде
4 августа 2026.

**Короткий ответ для тех, кто спешит:** понадобилось добавить три файла и
дописать три ключа к команде запуска тестов. Код тестов — ноль изменений.
...
`--collect`. И томов под отчёты в compose тоже не было. **Значит результаты
оставались внутри контейнера и умирали вместе с ним.** Единственный сигнал —
код возврата: «упало / не упало», без имени упавшего теста.
...
От оригинального отличается четырьмя местами:

```yaml
services:
  tests:
    # было build: — стало готовый образ, собранный отдельной конфигурацией
    image: proget.inc.elara.local/<фид>/openide-postgres-integration-tests:1.0.0
    depends_on:
      postgres:
...
Убрано: `build:`, `container_name`, `restart: always`. Добавлено: готовый образ
и три ключа выгрузки отчётов.
...
Про покрытие 4.44% — это не поломка, а арифметика: интеграционные тесты слоя
...
## Пять вещей, на которые наткнулись — и на которые наткнётесь вы
...
каталог вложений `results/_<host>_<date>/In/<host>/`. Глоб `results/**` заберёт
оба, и цифры удвоятся. Отсюда `results/*/…` в примере выше. (Ядро на всякий
случай считает одинаковые файлы один раз, но правильный глоб надёжнее.)

**3. `--collect:"XPlat Code Coverage"` работает даже при
`PrivateAssets="all"` у `coverlet.collector`** — это проверено, коллектор
доезжает до опубликованного бандла.
...
поэтому наш `.env` его перекрывает — но настоящий пароль туда класть нельзя:
он уедет в опубликованный артефакт сборки.

**5. Базе нужно вернуть капабилити.** Под снятыми привилегиями postgres падает
с `failed switching to 'postgres'`. Пяти из словаря хватает и для 18.1 —
проверено.
...
- [ ] глоб покрытия — `results/*/coverage.cobertura.xml`, а не `**`
- [ ] к соседним сервисам — по имени из compose, не `localhost`
- [ ] у зависимых сервисов есть `healthcheck`, у тестов — `depends_on … service_healthy`
...
остаются на машине навсегда. В первый этап такие наборы не входят — если это
ваш случай, приходите, будем решать отдельно.
