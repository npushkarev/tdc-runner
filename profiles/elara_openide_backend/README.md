...
| Набор | Что гоняет | Что нужно |
|---|---|---|
| `postgres_integration` | тесты слоя Infrastructure на PostgreSQL | образ, собранный из одного csproj. Прогнан на стенде |
| `all_tests` | все тесты решения, кроме web-набора | образ, собранный из ВСЕГО решения. Не прогонялся |

Второй набор требует изменения в вашем Dockerfile. Сейчас там собирается один
проект:

```
dotnet build ./tests/<один проект>.csproj -c Release
```
...
Иначе `dotnet test --no-build` не найдёт бинарники остальных проектов.

Web-набор исключён фильтром `FullyQualifiedName!~Backend.Web.IntegrationTests`.
Он создаёт контейнеры через `Docker.DotNet`, такие наборы пока не
поддерживаются.

Не готовы менять сборку: оставьте только `postgres_integration`, удалив папку
`all_tests`.
