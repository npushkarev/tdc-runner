...
| .NET-командам, «покажите на живом примере» | **[docs/case_dotnet_openide.md](docs/case_dotnet_openide.md)** — разбор настоящего проекта: как было, что изменили (три файла, ноль правок в тестах), что получилось, пять типовых граблей |
...
`profiles/<репозиторий>/` — готовые профили, подготовленные для конкретных
команд: копируются в корень их репозитория и там живут дальше. Сейчас там
`elara_openide_backend` (прогнан на стенде). Пример для тестов и смоука —
