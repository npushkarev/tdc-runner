...
| Кому | Что читать |
|---|---|
| тому, кто рассказывает разработчикам | **[docs/for_developers.md](docs/for_developers.md)**: что говорить, тонкости запуска, требования к тестам, сценарий показа на живой машине |
| разработчику, «просто скажите команду» | **[docs/cheatsheet.md](docs/cheatsheet.md)**: один экран, одна команда, частые ошибки |
...
| при работе с контрактом | `docs/test_cfg.xsd`: формат манифеста. Документация, исполняемая валидация это код |
| .NET-командам, «покажите на живом примере» | **[docs/case_dotnet_openide.md](docs/case_dotnet_openide.md)**: разбор настоящего проекта. Как было, что изменили (три файла, ноль правок в тестах), что получилось |
...
`profiles/<репозиторий>/` это готовые профили для конкретных
команд: копируются в корень их репозитория и там живут дальше. Сейчас там
`elara_openide_backend` (прогнан на стенде). Пример для тестов и смоука это
`tests/fixtures/demo_repo`, он один на оба применения.

Схемы лежат в `docs/in662_*.png`. Рядом редактируемые `.excalidraw` и генераторы
